...
def search(raw_query, page, per_page, types=['event', 'story', 'concept'], url_prefix=''):
    """
    Full-text search.
    ...
    for r in results:
        ...
        if (r['type'] == 'concept'):
            r['url'] = url_for(url_prefix+r['type'], slug=r['slug'])
        else:
            r['url'] = url_for(url_prefix+r['type'], id=r['id'])
